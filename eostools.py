...
setCAFPath()
#import cmsIO
from PhysicsTools.HeppyCore.utils.cmsIO import *
...
def runXRDCommand(path, cmd, *args):
    ...
    lfn = eosToLFN(path)
    #print "lfn:", lfn, cmd
    tokens = splitPFN(lfnToPFN(lfn))#cmsIO.splitPFN(lfnToPFN(lfn))
    
    command = ['xrd', tokens[1], cmd, tokens[2]]
    command.extend(args)
    runner = cmsFileManip()#cmsIO.cmsFileManip()
    # print ' '.join(command)
...
def runEOSCommand(path, cmd, *args):
    ...
    lfn = eosToLFN(path)
    pfn = lfnToPFN(lfn)
    tokens = splitPFN(pfn) #cmsIO.splitPFN(pfn)
...
def lfnToPFN( path, tfcProt = 'rfio'):
    ...
    entity = cmsFile( path, tfcProt )#cmsIO.cmsFile( path, tfcProt )
...
def xrdcp(src, dest):
    ...
    if isEOSDir(dest):
        ...
        if isDirectory(dest):
            tokens = splitPFN(pfn_dest)#cmsIO.splitPFN(pfn_dest)
